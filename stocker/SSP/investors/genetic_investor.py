        ...
        if isinstance(event, EventStockOpen):
            for company in self.companies.keys():
                ...
                if self.learning <= self.days:
                    self.ga[company] = tradingGA(self.min_price_list[company], self.max_price_list[company],
                        self.last_price_list[company], self.volume_list[company])
